    ...
    for i, name in enumerate(item_names):
        ...
        with target_col:
            with st.expander(f"📦 {name}", expanded=True):
                ...
                # 기존 월누계 (수정 가능) + 오늘 일합계 = 최종 월누계
                prev_total = month_totals_default[i]
                base_raw = cols[len(shift_labels) + 1].text_input("기존누계", value=str(prev_total), key=f"wl_base_{i}")
                try:
                    base_val = int(float(base_raw)) if base_raw.strip() else prev_total
                except (ValueError, TypeError):
                    base_val = prev_total
                running_month = base_val + daily_sum
                cols[len(shift_labels) + 1].caption(f"→ 월누계: **{running_month}**")
