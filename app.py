...
def _restore_user_role(uname: str):
    ...
    if uname in _admins or (not _admins and uname in ("user", "")):
        ...
        return
    try:
        from utils.supabase_db import get_app_user_by_name as _gabn
        _u = _gabn(uname)
        if _u:
        ...
        if submitted:
            ...
            if _user:
                ...
                st.session_state.update({
                    ...
                    "user_role": _user["role"], "user_dept": dept, "user_employee_name": name,
                })
                st.query_params["t"] = token
                st.rerun()
            else:
...
def page_attendance():
    ...
    elif "att_shift_type" not in st.session_state:
        st.session_state["att_shift_type"] = "4조3교대"
...
[data-testid="stScrollableElement"]::-webkit-scrollbar-thumb:hover { background: #94a3b8; }
    ...
    with _tab_sector:
        ...
        if sort_mode in ("섹터별", "품목별"):
            # ── 카드 그리드 + 아래 전체너비 펼침 ──
            _active_key = st.session_state.get("inv_active_group")
            _group_keys = [gk for gk, _ in _all_groups]
            if _active_key not in _group_keys:
                _active_key = None
                st.session_state.pop("inv_active_group", None)

            for _gi in range(0, len(_all_groups), 3):
                _row_slice = _all_groups[_gi:_gi + 3]
                _card_cols = st.columns(3)
                for _ci, (group_key, group_df) in enumerate(_row_slice):
                    cnt = len(group_df)
                    _is_active = (group_key == _active_key)
                    _sel_cnt = sum(1 for _l in group_df["lot"].tolist() if st.session_state.get(f"chk_{_l}", False))
                    _sel_badge = f" ✓{_sel_cnt}" if _sel_cnt else ""
                    _card_label = f"{'▼ ' if _is_active else ''}{group_key}  {cnt}드럼{_sel_badge}"
                    with _card_cols[_ci]:
                        if st.button(_card_label, key=f"inv_card_{group_key}",
                                     use_container_width=True, type="secondary"):
                            if _is_active:
                                st.session_state.pop("inv_active_group", None)
                            else:
                                st.session_state["inv_active_group"] = group_key
                            st.rerun()

                # 이 행(3개 카드)에 활성 그룹이 있으면 아래 전체너비로 펼침
                _active_in_row = next(
                    ((gk, gdf) for gk, gdf in _row_slice if gk == _active_key), None
                )
                if _active_in_row:
                    _akey, _adf = _active_in_row
                    st.markdown(
                        f"<div style='background:#F0F4FF;border-left:4px solid #1565C0;"
                        f"padding:10px 16px 4px;margin:4px 0 8px;border-radius:0 8px 8px 0;"
                        f"font-weight:700;color:#1565C0;'>{_akey} — {len(_adf)}드럼</div>",
                        unsafe_allow_html=True
                    )
                    _render_group_detail(_akey, _adf)
                    st.markdown("---")
        else:
...
def page_employee_admin():
    ...
    with st.expander("➕ 직원 등록", expanded=False):
        with st.form("emp_add_form"):
            ...
            with _c2:
                _add_name = st.text_input("이름", key="emp_add_name")
            with _c3:
                _add_pw = st.text_input("초기 비밀번호 (사번)", key="emp_add_pw")
            _add_submitted = st.form_submit_button("등록", type="primary", use_container_width=True)
        if _add_submitted:
            _add_name = _add_name.strip()
            if not _add_name or not _add_pw:
                st.error("이름과 비밀번호를 입력하세요.")
            else:
                try:
                    register_app_user(_add_dept, _add_name, _add_pw)
                    st.success(f"'{_add_name}' 등록 완료!")
                    st.rerun()
    ...
    for _d in _EMPLOYEE_DEPTS:
        for _n, _p in dict(st.secrets.get(_d, {})).items():
            _mig_candidates.append((_d, _n, _p))
    if _mig_candidates:
        with st.expander("⬆️ 기존 직원 일괄 등록 (secrets → Supabase)", expanded=False):
            st.caption("secrets.toml에 등록된 직원을 Supabase로 한번에 이전합니다. 이미 등록된 계정은 건너뜁니다.")
            if st.button("일괄 등록 실행", type="primary"):
                _ok, _skip = 0, 0
                for _d, _n, _p in _mig_candidates:
                    try:
                        register_app_user(_d, _n, _p)
                        _ok += 1
                    except ValueError:
                        _skip += 1
                    except Exception:
    ...
    for _dept_name, _emps in _dept_groups.items():
        ...
        for _e in _emps:
            _col_n, _col_pw, _col_del = st.columns([3, 2, 1])
            with _col_n:
                st.markdown(f"👤 {_e['name']}")
            with _col_pw:
                with st.popover("🔑 비밀번호 초기화"):
                    with st.form(f"pw_reset_{_dept_name}_{_e['name']}"):
                        _new_pw = st.text_input("새 비밀번호", key=f"npw_{_dept_name}_{_e['name']}")
                        if st.form_submit_button("초기화", type="primary"):
                            if _new_pw.strip():
                                reset_app_user_password(_dept_name, _e["name"], _new_pw.strip())
                                st.success("변경 완료")
                            else:
                                st.error("비밀번호를 입력하세요.")
            with _col_del:
                if st.button("삭제", key=f"del_{_dept_name}_{_e['name']}", type="secondary"):
                    delete_app_user(_dept_name, _e["name"])
                    st.rerun()
        st.markdown("---")
