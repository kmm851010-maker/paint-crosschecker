...
### STEP 1: "신규" 헤더 찾기
표 헤더에서 "신규"라고 적힌 열의 위치를 모두 파악하세요.
블록이 여러 개이므로 "신규" 열도 여러 개 있습니다.

### STEP 2: 신규 열에 숫자가 있는 셀만 수집
각 "신규" 열을 위에서 아래로 스캔하세요.
- 셀에 숫자가 있으면 → 수집 대상 (예: 16, 3, 18, "16(수)", "3(화)")
- 셀이 비어있거나 "-" 이거나 0이면 → **즉시 건너뜀. 이 셀은 무시.**

### STEP 3: 수집된 셀에서 왼쪽으로 역추적
STEP 2에서 수집된 각 셀에 대해:
- 같은 행에서 **왼쪽으로** 이동하여 해당 블록의 품목코드와 제조사를 찾으세요.
- 품목코드: 신규 셀 기준 왼쪽 3칸 (영문+숫자 7자리, 예: P7G342E)
- 제조사: 신규 셀 기준 왼쪽 2칸 (한글 2~3글자, 예: 고려, 대한)
- 해당 블록의 레이어명: 그 열 그룹의 헤더 (TOP, BACK, 프라이머/클리어 등)

### 절대 금지
- "재고" 열의 숫자를 quantity에 넣지 마세요.
- "신규"에 숫자가 없는 행은 절대 추출하지 마세요.
