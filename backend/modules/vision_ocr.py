...
**다중 섹션 처리 (매우 중요!):**
- 이미지에 5CCL, 6CCL 등 여러 구역이 상하로 나뉘어 있으면 모든 구역을 하나의 rows 배열에 합쳐서 반환하세요.
- 새 구역 시작 시 구역 레이블(예: "6CCL")을 첫 셀에 넣은 행을 rows에 추가하세요. (나머지 셀은 null)
- 두 번째 구역의 컬럼 헤더 행(TOP, 회사, 재고... 반복)은 rows에 포함하지 않아도 됩니다.
