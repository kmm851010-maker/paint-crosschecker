...
UNIVERSAL_DOC_PROMPT = """당신은 제조업 생산계획표에서 신규 입고 대상 품목만 정밀 추출하는 전문 AI입니다.

이 이미지는 도료(페인트) 생산계획표입니다.
표는 가로로 여러 블록(TOP, BACK, 프라이머/클리어, 프라이머 등)이 반복되며,
각 블록은 [품목코드, 회사, 재고, 신규] 4개 열로 구성됩니다.

## 추출 방법 (반드시 이 순서대로!)

### STEP 1: "재고" 열을 모두 찾아서 완전히 배제
표 헤더에서 "재고"라고 적힌 열의 위치를 모두 파악하세요.
**"재고" 열과 그 아래의 모든 숫자는 완전히 무시하세요. 절대 추출 대상이 아닙니다.**

### STEP 2: "신규" 열만 찾기
"재고"를 배제한 후, 표 헤더에서 "신규"라고 적힌 열의 위치를 모두 파악하세요.
블록이 여러 개이므로 "신규" 열도 여러 개 있습니다.

### STEP 3: 신규 열에 숫자가 있는 셀만 수집
각 "신규" 열을 위에서 아래로 스캔하세요.
- 셀에 숫자가 있으면 → 수집 대상 (예: 16, 3, 18, "16(수)", "3(화)")
- 셀이 비어있거나 "-" 이거나 0이면 → **즉시 건너뜀**
- 숫자가 아닌 문자만 있으면(예: "T") → **건너뜀**

### STEP 4: 수집된 셀에서 왼쪽으로 역추적
STEP 3에서 수집된 각 셀에 대해:
- 같은 행에서 **왼쪽으로** 이동하여 해당 블록의 품목코드와 제조사를 찾으세요.
- 품목코드: 신규 셀에서 왼쪽으로 가며 영문+숫자 7자리 코드를 찾으세요 (예: P7G342E)
- 제조사: 품목코드 바로 오른쪽 셀의 한글 텍스트 (예: 고려, 대한)
- 레이어명: 그 열 그룹의 상단 헤더 (TOP, BACK, 프라이머/클리어 등)
  ...
  "items": [
    {
      ...
      "weight_kg": 0,
      "manufacturer": "제조사",
      "extra_info": "레이어명(TOP/BACK/프라이머/클리어 등)",
      "note": "비고 또는 빈문자열"
    }
  ]
}
