"""
캡처 이미지 테이블 추출 모듈
Claude Vision API를 사용하여 ERP/엑셀 캡처 이미지에서
표 데이터를 구조화된 JSON으로 변환합니다.
"""

import base64

import anthropic

from utils.helpers import detect_media_type
from modules.vision_ocr import _parse_json_response as parse_json_response


TABLE_EXTRACT_PROMPT = """당신은 이미지 속 표(테이블)를 완벽하게 디지털 데이터로 변환하는 전문가입니다.

이 이미지에서 표를 찾아 **모든 셀의 데이터를 한 글자도 빠짐없이** 정확히 추출하세요.

## 추출 규칙

### 1. 헤더(컬럼명) 추출
- 이미지에서 **맨 처음 나오는** 표의 헤더 행을 컬럼명으로 사용하세요.
- 헤더가 여러 줄로 병합된 경우 하위 컬럼명을 사용하세요.
- 헤더 텍스트는 원본 그대로 유지하세요.

### 2. 다중 섹션 처리 (매우 중요!)
- 이 이미지에는 **5CCL, 6CCL** 등 여러 구역(섹션)이 상하로 나뉘어 있을 수 있습니다.
- **모든 섹션의 모든 데이터 행을 하나의 rows 배열에 합쳐서** 반환하세요.
- 새 섹션이 시작될 때 섹션 레이블(예: "6CCL")을 **별도 행으로 rows에 포함**하세요.
  예: 6CCL 섹션이 시작되면 → ["6CCL", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""] 처럼 첫 셀에 레이블, 나머지는 빈 문자열
- 두 번째 섹션의 컬럼 헤더 행(TOP, 회사, 재고, 신규... 반복)은 rows에 포함하지 않아도 됩니다.

### 3. 데이터 행 추출
- 모든 행을 빠짐없이 추출하세요. 행 하나라도 누락하면 안 됩니다.
- 빈 셀은 빈 문자열("")로 표시하세요.

### 4. 데이터 타입 변환 (매우 중요!)
- 품목코드/색상코드: 영문+숫자 조합은 문자열 그대로 유지 (예: "P7A052D")
- 순수 숫자만 있는 셀: JSON 숫자형으로 변환 (예: 15, 300, 18.5)
- 천 단위 콤마가 있는 숫자: 콤마 제거 후 숫자형으로 (예: "1,250,000" → 1250000)
- **숫자+괄호 텍스트 (예: "16(수)", "3(화)", "22(목)")**: 문자열 그대로 유지! 숫자로 변환하지 마세요!
- **"창고3", "13중12", "위생산" 등 텍스트 포함 셀**: 문자열 그대로 유지
- 날짜, 텍스트: 문자열 그대로 유지
- 비슷한 글자 주의: 0(영)과 O(오), 1(일)과 I(아이), 5와 S, 8과 B

### 5. 합계/소계 행
- "합계", "소계", "TOTAL" 등의 행이 있으면 그대로 포함하세요.

반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트 없이 JSON만:

{
  "headers": ["컬럼1", "컬럼2", "컬럼3"],
  "rows": [
    ["값1", 숫자값, "값3"],
    ["6CCL", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""],
    ["값1", 숫자값, "값3"]
  ]
}
"""


def extract_table_from_image(
    image_bytes: bytes,
    file_name: str,
    api_key: str,
    model: str = "claude-opus-4-8",
) -> dict:
    """
    캡처 이미지 또는 PDF에서 표 데이터를 추출합니다.

    Returns:
        {"headers": [...], "rows": [[...], ...]}
    """
    client = anthropic.Anthropic(api_key=api_key)
    b64_data = base64.standard_b64encode(image_bytes).decode("utf-8")
    media_type = detect_media_type(file_name)

    is_pdf = media_type == "application/pdf"
    file_content = {
        "type": "document" if is_pdf else "image",
        "source": {
            "type": "base64",
            "media_type": media_type,
            "data": b64_data,
        },
    }

    response = client.messages.create(
        model=model,
        max_tokens=16000,
        messages=[
            {
                "role": "user",
                "content": [
                    file_content,
                    {
                        "type": "text",
                        "text": TABLE_EXTRACT_PROMPT,
                    },
                ],
            }
        ],
    )

    text = ""
    for block in response.content:
        if getattr(block, "type", "") == "text":
            text = getattr(block, "text", "")
            if text:
                break

    if not text:
        raise ValueError("테이블 추출 응답에서 텍스트를 찾을 수 없습니다.")

    data = parse_json_response(text)

    if "headers" not in data or "rows" not in data:
        raise ValueError("응답에 headers 또는 rows가 없습니다.")

    return data
